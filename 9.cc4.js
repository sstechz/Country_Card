...
// //  Be careful the new API has its limitation ie 1 request per second


// -------------------------------------------------------------------------------------------------
//  Creating Promises
//  Fetching location using geolocation_api
...
function callCountryDetailApi(country_code) {
    const request = fetch(`https://restcountries.com/v3.1/alpha/${country_code}`)
    .then(response => {
        const str = response.json();
        //console.log(str);
        return str;
...
function whereAmI(lat, long){
    ...
    .then(function(data) {
        const info = data;
        //console.log(info.address.country_code);
        callCountryDetailApi(info.address.country_code)
    });
}

const getPosition = new Promise(function(response, error){
